...
// Problem 1
// Gather up all of the emails so they can be included in a mailing list
let emailList = runners.map(runner => runner.email);
console.log(emailList);

// Problem 2
// Count the number of shirts needed in each size so an order can be placed (return an array like this: ['XS: 10', 'S: 20', etc])

function countShirts(arr, sizeArray) {
    let result = [];
    sizeArray.forEach(size => {
        let numShirts = arr.filter(arrItem => arrItem.shirt_size === size).length;
        result.push(`${size}: ${numShirts}`);
    });
    return result;
}

const shirtSizes = ['XS', 'S', 'M', 'L', 'XL', '2XL', '3XL'];
console.log(countShirts(runners, shirtSizes));


// Problem 3
// Construct an array of companies sorted in order of donation amount, largest to smallest, so the organizers know who to thank first

let sortedDonors = runners.sort((runner1, runner2) => runner2.donation - runner1.donation).map(runner => runner.company_name);
